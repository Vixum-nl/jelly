...
$(document).ready(function () {
	...
	console.log('This website is build by PinkWhale. (https://pinkwhale.io)');

	$('.sub-nav#randa').hide();

	$('ul[toggleArticles]').hide();
	$('ul[toggleArticles=1]').show();

	$('#toggleSubArticles').on('click', function () {
		$('.sub-nav#randa').toggle();
	});

	// For top navigation "Research & Articles".
	$('a[toggleArticle]').hover(function () {
		var ToOpen = $(this).attr('toggleArticle');
		$('ul[toggleArticles]').hide();
		$('ul[toggleArticles=' + ToOpen + ']').show();
	});
});
